...
import styles, { C } from './HeavyItem.styles';
    ...
    <View style={styles.header}>
      ...
      <View style={styles.headerInfo}>
        <View style={styles.nameRow}>
          ...
          {isVerified && <IconVerified fill="#3B82F6" width={14} height={14} style={{ marginLeft: 4 }} />}
        </View>
        <Text style={styles.handleTime}>{userHandle} · {timeAgo}</Text>
      </View>
...
const ImageGrid = memo(({ images, onImagePress }: {
  images: string[];
  ...
  const renderItem = (index: number, customStyle?: any) => (
    <AppPress
      style={customStyle || styles.imageFill}
...
export default memo(SocialPostCard, (prev, next) =>
  prev.item.id === next.item.id && prev.onImagePress === next.onImagePress
);
